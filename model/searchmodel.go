...
type (
	SearchModel interface {
		SearchPublicMediaAssets(ctx context.Context, query string, limit int64) ([]*MediaAsset, error)
		SearchPublicPosts(ctx context.Context, query string, limit int64) ([]*Post, error)
		SearchPublicAlbums(ctx context.Context, query string, limit int64) ([]*Album, error)
...
func (m *defaultSearchModel) SearchPublicMediaAssets(ctx context.Context, query string, limit int64) ([]*MediaAsset, error) {
...
order by
  ...
  ma.`+"`id`"+` desc
limit ?`, qualifiedRows(mediaAssetRows, "ma"))
...
func (m *defaultSearchModel) SearchPublicPosts(ctx context.Context, query string, limit int64) ([]*Post, error) {
	query = strings.TrimSpace(query)
	if query == "" {
		return []*Post{}, nil
	}
...
func (m *defaultSearchModel) SearchPublicAlbums(ctx context.Context, query string, limit int64) ([]*Album, error) {
	query = strings.TrimSpace(query)
	if query == "" {
		return []*Album{}, nil
	}
...
order by
  ...
  coalesce(es.`+"`view_count`"+`, 0) desc,
  a.`+"`id`"+` desc
limit ?`, qualifiedRows(albumRows, "a"))
