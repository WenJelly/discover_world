...
func (m *defaultPostModel) FindByIDs(ctx context.Context, ids []uint64) (map[uint64]*Post, error) {
	...
	query := fmt.Sprintf("select %s from %s where `id` in (%s) and `status` <> 'deleted' and `deleted_at` is null", postRowsWithDefaultScore, m.table, modelutil.InPlaceholders(len(ids)))
	var rows []*Post
...
func (m *defaultPostModel) FindPublicByAuthorsBeforeCursor(ctx context.Context, authorIDs []uint64, beforeID uint64, limit int64) ([]*Post, error) {
	authorIDs = modelutil.UniquePositiveIDs(authorIDs)
	if len(authorIDs) == 0 {
		return []*Post{}, nil
	}
	if limit <= 0 {
	...
	conditions := []string{
		fmt.Sprintf("`user_id` in (%s)", modelutil.InPlaceholders(len(authorIDs))),
		"`status` = 'active'",
...
func (m *defaultPostModel) FindByUserBeforePinCursor(ctx context.Context, userID uint64, visibleValues []string, cursor PostPinCursor, limit int64) ([]*Post, error) {
	...
	conditions = append(conditions, fmt.Sprintf("`visibility` in (%s)", modelutil.InPlaceholders(len(visibleValues))))
