...
func (m *customUserProfileModel) FindByUserIDs(ctx context.Context, userIDs []uint64) (map[uint64]*UserProfile, error) {
	...
	userIDs = modelutil.UniquePositiveIDs(userIDs)
	if len(userIDs) == 0 {
		return resp, nil
	}
	...
	query := fmt.Sprintf("select %s from %s where `user_id` in (%s)", userProfileRows, m.table, modelutil.InPlaceholders(len(args)))
