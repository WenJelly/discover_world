...
func (m *customMediaObjectModel) FindOriginalByAssetIDs(ctx context.Context, assetIDs []uint64) (map[uint64]*MediaObject, error) {
	...
	assetIDs = uniquePositiveIDs(assetIDs)
	if len(assetIDs) == 0 {
		return resp, nil
	}
	...
	query := fmt.Sprintf("select %s from %s where `asset_id` in (%s) and `object_role` = 'original' and `status` = 'active'", mediaObjectRows, m.table, inPlaceholders(len(args)))
