...
func (m *customReactionModel) FindActiveTargetIDsByUser(ctx context.Context, userID uint64, targetType string, targetIDs []uint64, reactionType string) (map[uint64]bool, error) {
	...
	query := fmt.Sprintf("select `target_id` from %s where `user_id` = ? and `target_type` = ? and `reaction_type` = ? and `status` = 1 and `target_id` in (%s)", m.table, modelutil.InPlaceholders(len(targetIDs)))
	var rows []struct {
...
func (m *customReactionModel) FindActiveUserIDsByTargets(ctx context.Context, targetType string, targetIDs []uint64, reactionType string, limitPerTarget int64) (map[uint64][]uint64, error) {
	...
	query := fmt.Sprintf("select `target_id`, `user_id` from %s where `target_type` = ? and `reaction_type` = ? and `status` = 1 and `target_id` in (%s) order by `target_id` asc, `updated_at` desc, `id` desc", m.table, modelutil.InPlaceholders(len(targetIDs)))
	var rows []struct {
		TargetId uint64 `db:"target_id"`
