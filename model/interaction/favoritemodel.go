...
func (m *customFavoriteModel) FindActiveTargetIDsByUser(ctx context.Context, userID uint64, targetType string, targetIDs []uint64) (map[uint64]bool, error) {
	...
	query := fmt.Sprintf("select `target_id` from %s where `user_id` = ? and `target_type` = ? and `status` = 1 and `target_id` in (%s)", m.table, modelutil.InPlaceholders(len(targetIDs)))
	var rows []struct {
