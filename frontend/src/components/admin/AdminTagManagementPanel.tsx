    ...
    <section className="space-y-5" aria-labelledby="admin-tags-title">
      ...
      <div className="overflow-hidden border-y border-border bg-background sm:rounded-xl sm:border">
        ...
        {!loading && error && tags.length === 0 ? (
          <div className="flex min-h-52 flex-col items-center justify-center px-6 text-center"><Tags className="size-6 text-muted-foreground" /><p className="mt-3 text-sm font-medium text-foreground">标签列表加载失败</p><p className="mt-1 text-sm text-muted-foreground">{error}</p><Button type="button" variant="outline" className="mt-4" onClick={() => void loadTags()}>重新加载</Button></div>
        ) : !loading && !error && tags.length === 0 ? (
        ...
        <div className="flex items-center justify-between border-t border-border px-3 py-2">
          <Button type="button" variant="ghost" size="sm" disabled={pageNum <= 1 || loading} onClick={() => { setHighlightedTagId(""); setPageNum((current) => Math.max(1, current - 1)); }}><ChevronLeft className="size-4" />上一页</Button>
          <span className="text-xs text-muted-foreground">{pageNum} / {pageCount}</span>
          <Button type="button" variant="ghost" size="sm" disabled={pageNum >= pageCount || loading} onClick={() => { setHighlightedTagId(""); setPageNum((current) => Math.min(pageCount, current + 1)); }}>下一页<ChevronRight className="size-4" /></Button>
        </div>
      </div>
      ...
      <Dialog open={Boolean(editTag)} onOpenChange={handleEditOpenChange}>
        <DialogContent className="max-h-[85dvh] max-w-xl overflow-y-auto">
          ...
          <DialogFooter><Button type="button" variant="outline" disabled={savingEdit} onClick={() => handleEditOpenChange(false)}>取消</Button><Button type="button" disabled={!editValid || savingEdit} onClick={() => void handleUpdateTag()}>{savingEdit ? <Loader2 className="size-4 animate-spin" /> : null}保存标签</Button></DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={Boolean(mergeSource)} onOpenChange={handleMergeOpenChange}>
        <DialogContent className="max-h-[85dvh] max-w-xl overflow-y-auto">
          ...
          {mergeSource ? <div className="space-y-5">
            ...
            <div className="space-y-1.5"><Label htmlFor="merge-target-search">搜索目标标签</Label><div className="flex gap-2"><Input id="merge-target-search" value={targetSearch} onChange={(event) => setTargetSearch(event.target.value)} placeholder="输入标签名称" onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void searchTargets(); } }} /><Button type="button" variant="outline" disabled={targetLoading} onClick={() => void searchTargets()}>{targetLoading ? <Loader2 className="size-4 animate-spin" /> : <Search className="size-4" />}搜索</Button></div></div>
            {targetError ? <p className="text-sm text-destructive">{targetError}</p> : null}
            {targetResults.length > 0 ? <div className="max-h-52 overflow-y-auto rounded-lg border border-border" aria-label="目标标签搜索结果">{targetResults.map((tag) => <button key={tag.id} type="button" onClick={() => setMergeTarget(tag)} className={cn("flex w-full items-center justify-between gap-3 border-b border-border px-3 py-3 text-left last:border-b-0 hover:bg-muted/45 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring", mergeTarget?.id === tag.id && "bg-muted")}><span><span className="block text-sm font-medium text-foreground">{tag.name}</span><span className="text-xs text-muted-foreground">{tag.slug} · {tagTypeLabel(tag.tagType)}</span></span><span className="text-xs text-muted-foreground">ID {tag.id}</span></button>)}</div> : null}
          ...
          <DialogFooter><Button type="button" variant="outline" disabled={merging} onClick={() => handleMergeOpenChange(false)}>取消</Button><Button type="button" variant="destructive" disabled={!canMerge} onClick={() => void handleMerge()}>{merging ? <Loader2 className="size-4 animate-spin" /> : <Merge className="size-4" />}确认合并</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </section>
  );
