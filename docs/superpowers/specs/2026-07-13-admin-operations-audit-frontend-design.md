...
列表项显示管理员、中文操作名称、目标、原因摘要和时间。首次加载自动选择当前页第一条；筛选或翻页后，如果当前记录不在结果中，则选择新列表第一条。
...
点击列表项时同步更新 `logId`。刷新 `/admin?tab=audit&logId=<日志ID>` 仍能打开同一条记录。`logId` 不在当前列表页时，详情接口独立加载，不强制改变当前筛选条件。切换离开审计栏目时移除 `logId`。
